...
Commerce Cloud Data Connector is an operation-based connector, which means that when you add the connector to your flow, you need to configure a specific operation for the connector to perform. Shop API Connector supports three different types of configuration; this example uses the Customer Auth Connection Provider configuration.
...
image::data-api/data-connector-example/shop-connector-http-listener-config.jpg[]

[start = 2]
. Add Operation, Assignment Search operations to the flow.
...
. You should see the seach data in the output.
