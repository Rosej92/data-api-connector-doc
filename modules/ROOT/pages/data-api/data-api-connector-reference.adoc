...
==== Connection Types
[[DataOauth2Configuration_Oauth2Application]]
===== Oauth2 Application Connection Provider
...
The query attribute specifies a complex query that can be used to narrow down the search. Attributes are grouped into different buckets. The following is a list of searchable attributes with their corresponding buckets:
      ...
      Note that only searchable attributes can be used in sorting. This operation makes an HTTP POST request to the /pricing/assignments/{{version}}/organizations/{{organizationId}}/assignments endpoint
